...
- Language: [[https://imagemagick.org/script/magick-vector-graphics.php][Magick Vector Graphics]]
  - no concept of containers or sets of commands
  - uses the concept of "Graphics Contexts"
  - ignores whitespace between commands
  - no loops / no control flow
  - .mvg
...
- Since version 7, supports fx() inline in -draw or other commands, in the form of:
  #+begin_src sh
    %[fx:w*.9]
  #+end_src
- user variable names:
...
|--------+--------+----------------------------------------|
| extent |  100x0 | outputs exact size given               |
| crop   |  100x0 | outputs exact size given, and the rest |
|        | +100+0 | removes from left side                 |
| shave  |  100x0 | removes from both sides                |
|--------+--------+----------------------------------------|
